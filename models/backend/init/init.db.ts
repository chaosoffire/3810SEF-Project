...
import {
	getUserByUsername,
	hasAdminUser,
} from "../database/model/user/user.repository";
...
async function ensureInitialAdminUser(): Promise<void> {
	try {
		...
		try {
			...
			console.error(
				"INIT_ADMIN_USERNAME or INIT_ADMIN_PASSWORD not set; skipping automatic admin bootstrap.",
			);
			exit(1);
		}
		...
		if (username.length < 8 || username.length > 32) {
			console.error(
				"INIT_ADMIN_USERNAME must be 8-32 characters; skipping automatic admin bootstrap.",
			);
			exit(1);
		}
		...
		if (password.length < 12 || password.length > 64) {
			console.error(
				"INIT_ADMIN_PASSWORD must be 12-64 characters; skipping automatic admin bootstrap.",
			);
			exit(1);
		}
		...
		if (existing) {
			console.error(
				`Bootstrap admin username "${username}" already exists but is not an admin. Aborting startup to avoid privilege escalation.`,
			);
			exit(1);
		}
