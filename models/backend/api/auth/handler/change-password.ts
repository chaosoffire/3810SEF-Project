...
export async function changePasswordHandler(req: Request, res: Response) {
  try {
    ...
    if (!oldPassword.length || !newPassword.length) {
      return res.status(400).json({ success: false, error: 'Passwords cannot be empty' });
    }
    if (newPassword.length < 12 || newPassword.length > 64) {
      return res.status(400).json({ success: false, error: 'New password must be 12-64 characters long' });
    }
    ...
    type DbUserCredential = { credential?: { passwordHash?: string } } | null;
