...
export async function createSessionToken(username: string, ttlMs?: number): Promise<string> {
  const now = Date.now();
...
export async function verifySessionToken(token: string): Promise<SessionPayload | null> {
  try {
    ...
    if (!payload || typeof payload.username !== 'string' || typeof payload.create_at !== 'number' || typeof payload.timeout !== 'number') {
      return null;
    }
    return payload;
  } catch {
