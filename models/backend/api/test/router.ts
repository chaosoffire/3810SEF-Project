...
testRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      return res.status(500).json({ success: false, error: 'Failed to create test user' });
    }
  }
);
...
testRouter.get(
  '/users/:username',
  param('username').isString().isLength({ min: 1, max: 64 }),
  handleValidationErrors,
  async (req, res) => {
    ...
    try {
      ...
      if (!u || u.role !== 'test') return res.status(404).json({ success: false, error: 'User not found' });
...
testRouter.put(
  ...
  async (req, res) => {
    ...
    try {
      const user = await userRepo.getUserByUsername(username);
      const u = user ? JSON.parse(JSON.stringify(user)) : null;
      if (!u || u.role !== 'test') return res.status(404).json({ success: false, error: 'Test user not found' });
...
testRouter.delete(
  '/users/:username',
  param('username').isString().isLength({ min: 8, max: 32 }),
  handleValidationErrors,
  async (req, res) => {
    const username = req.params.username;
    try {
      ...
      if (u.role !== 'test') return res.status(403).json({ success: false, error: 'Only test users can be deleted' });
      await userRepo.deleteUserByUsername(username);
      return res.json({ success: true });
    } catch (err: any) {
      if (err?.name === 'CastError') return res.status(400).json({ success: false, error: 'Invalid username' });
