...
async function POSTordersHandler(req: Request, res: Response) {
  ...
  try {
    ...
    if (type === 'refund') {
      ...
      const notOwnedBooks = uniqueBookIds.filter(bookId => !ownedBookSet.has(bookId));
      
      if (notOwnedBooks.length > 0) {
        return res.status(400).json({ 
          success: false, 
          error: 'Book cannot be refunded as not owned',
          details: { notOwnedBooks }
        });
      }
      
      // All books are owned, proceed with refund
    await createOrder(username, { books: validObjectIds, type: 'refund' });
      ...
      const alreadyOwnedBooks = uniqueBookIds.filter(bookId => ownedBookSet.has(bookId));
      
      if (alreadyOwnedBooks.length > 0) {
        return res.status(400).json({ 
          success: false, 
          error: 'Book cannot be bought twice',
          details: { alreadyOwnedBooks }
        });
      }
      
      // None of the books are owned, proceed with purchase
      await createOrder(username, { books: validObjectIds, type: 'buy' });
      return res.status(200).json({ 
        success: true, 
