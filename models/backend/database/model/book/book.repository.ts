...
import { BookDocument, BookModel } from "../schema/bookSchema";
...
export async function createBook(book: BookDocument): Promise<true> {
  const model = MongoDBManager.getInstance().getModel<BookDocument>(BOOK_COLLECTION, bookSchema);
...
export async function getBooksByField(params: BookParam[]): Promise<BookResult | null> {
  ...
  if (Object.keys(searchConfig).length === 0) {
    ...
    return books ? { data: JSON.parse(JSON.stringify(books)), count: books.length } : null;
  }

  const andConditions: any[] = [];

  if (searchConfig._id && searchConfig._id.length > 0) {
    andConditions.push({ _id: { $in: searchConfig._id } });
  }
  ...
  if (searchConfig.title && searchConfig.title.length > 0) {
    const titleOrConditions = searchConfig.title.map(title => {
      ...
      return { title: { $regex: sanitized, $options: 'i' } };
    });
    if (titleOrConditions.length > 0) {
      andConditions.push({ $or: titleOrConditions });
    }
  }
  ...
  if (searchConfig.author && searchConfig.author.length > 0) {
    const authorOrConditions = searchConfig.author.map(author => {
      ...
      return { author: { $regex: sanitized, $options: 'i' } };
    });
    if (authorOrConditions.length > 0) {
      andConditions.push({ $or: authorOrConditions });
    }
  }
  ...
  if (searchConfig.description && searchConfig.description.length > 0) {
    const descriptionOrConditions = searchConfig.description.map(desc => {
      ...
      return { description: { $regex: sanitized, $options: 'i' } };
    });
    if (descriptionOrConditions.length > 0) {
      andConditions.push({ $or: descriptionOrConditions });
    }
  }

  if (searchConfig.genres && searchConfig.genres.length > 0) {
    andConditions.push({ genres: { $in: searchConfig.genres } });
  }

  if (searchConfig.publishedYear && searchConfig.publishedYear.length > 0) {
    andConditions.push({ publishedYear: { $in: searchConfig.publishedYear } });
  }

  if (searchConfig.minPrice !== undefined || searchConfig.maxPrice !== undefined) {
  ...
  if (searchConfig.sortBy) {
    ...
    mongooseQuery = mongooseQuery.sort({ [searchConfig.sortBy]: sortOrder });
  }
...
export async function getBooksByIds(bookIds: string[]): Promise<BookDocument[] | null> {
  const model = MongoDBManager.getInstance().getModel<BookDocument>(BOOK_COLLECTION, bookSchema);
  const books = await model.find({ _id: { $in: bookIds } }).lean();
  return books ? JSON.parse(JSON.stringify(books)) : null;
}

export async function updateBookById(bookId: string, updateData: Partial<BookDocument>): Promise<true> {
  const model = MongoDBManager.getInstance().getModel<BookDocument>(BOOK_COLLECTION, bookSchema);
...
export const WithPriceRange = (minPrice: number, maxPrice: number): BookParam => {
...
export const WithSort = (sortBy: keyof BookDocument, sortOrder?: SortOrder): BookParam => {
  const result = z
