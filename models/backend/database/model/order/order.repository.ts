...
import { IOrder, orderSchema } from '../schema/orderSchema';
...
export async function createOrder(
  username: string, 
  orderData: { books: mongoose.Types.ObjectId[]; type: 'buy' | 'refund' },
): Promise<void> {
  const orderModel = MongoDBManager.getInstance().getModel(ORDER_COLLECTION, orderSchema);
  const userModel = MongoDBManager.getInstance().getModel(USER_COLLECTION, userSchema);
...
export async function deleteOrderById(orderId: string): Promise<true> {
  const model = MongoDBManager.getInstance().getModel(ORDER_COLLECTION, orderSchema);
  await model.deleteOne({ _id: orderId });
