...
const searchFormEl = document.querySelector("#search-form") as HTMLFormElement | null;
...
searchFormEl?.addEventListener('submit',async(e:SubmitEvent)=>{
    ...
    const keywordString: string = keyword === ""? "" : `|title.${keyword}|author.${keyword}|description.${keyword}`;

    const priceRangestring:string = priceFilterValue[String(formData.get('price')||"any")];
    
    const sortBy:string = String(formData.get('sort')||"title");
    
    const category:string = String(formData.get('category')||"any");
    const categoryString:string = category === "any"? "":`|genres.${category}`;
