...
const switchMenuPage = async(
    ...
    const currentActiveItem = document.querySelector(".home-page-item.active") as HTMLLIElement | null;
    const currentActivePage:number = currentActiveItem ? parseInt(currentActiveItem.dataset.pageIndex as string) : 0;
    ...
    const hiddenInput = document.querySelector("#base-book-query") as HTMLInputElement | null;

    if (hiddenInput) {
        let query = hiddenInput.value as string;
        ...
        console.log(`Navigating to page ${nextPageNumber} with query: ${cleanQuery}`);
        
        window.location.href = `/page/content?state=home&requestQuery=${cleanQuery}`; 
    } else {
        console.error("CRITICAL ERROR: Hidden base-book-query input not found. Cannot navigate.");
    }
};
...
const prev = async(
    ...
    const currentItem = document.querySelector(".home-page-item.active") as HTMLLIElement | null;
    if (!currentItem) return; 
    
    const currentPageNumber: number = parseInt(currentItem.dataset.pageIndex as string);
...
document.addEventListener("DOMContentLoaded", () => {
    try {
        const listEl = document.querySelector(".home-pagination") as HTMLUListElement | null;
        const pageItemEl = document.querySelectorAll<HTMLLIElement>(".home-page-item");
        const previousEl = document.querySelector(".home-page-item-previous") as HTMLLIElement | null;
        const nextEl = document.querySelector(".home-page-item-next") as HTMLLIElement | null;

        if (!listEl) throw new Error("Pagination container not found.");
        
        const totalPage: number = parseInt(listEl.dataset.totalPage as string || '0');

        // Only attach listeners if total pages > 1
        if (totalPage > 1) {
            if (!previousEl || !nextEl) throw new Error("Previous/Next buttons missing.");
