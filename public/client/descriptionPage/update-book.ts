const saveBtnEl = document.querySelector("#save-btn") as HTMLButtonElement | null;
        ...
        try {
            ...
            if (response.ok && data.success) {
                ...
                if (isAdding && data.book && data.book._id) {
                    ...
                    const bookCoverElement = document.getElementById("book-cover") as HTMLImageElement | null;
                    if (bookCoverElement) {
                ...
                const message = data.message || `Failed to ${isAdding ? "create" : "update"} the book`;
                alert(`Error: ${message}`);
            }
        } catch (error: any) {
            console.error(`Error ${isAdding ? "creating" : "updating"} the book:`, error);
...
if (priceInput) {
    priceInput.addEventListener("input", function(e: Event): void {
    ...
    if (firstDot !== -1) {
        const afterDot = sanitized.substring(firstDot + 1).replace(/\./g, "");
    ...
    priceInput.addEventListener("blur", function(e: Event): void {
        const target = e.target as HTMLInputElement;
        let value: string = target.value;
