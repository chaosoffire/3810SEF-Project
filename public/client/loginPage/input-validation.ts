...
function signUpValidation(e:SubmitEvent): void{
        ...
        if(id === "" || password === ""){
            ...
            signUpStatusEl_v!.innerText = "Please provide an id within length of 8 and 32.";
            return;
        }else if(!id.match(/^[A-Za-z][_A-Za-z0-9]{7,31}$/)){
            signUpStatusEl_v!.innerText = "Username must starts with a letter, and contains only number, character and underscore";
            return;
        }else if(password.length < 12 || password.length > 64){
            signUpStatusEl_v!.innerText = "Password length must be within 12 and 64.";
            return;
        }else if(!password.match(/^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])[A-Za-z0-9]{12,64}$/)){
            signUpStatusEl_v!.innerText = "Password must include an uppercase, lowercase, number";
            return;
        }
    signUpIDEl!.value = id;
