...
window.onload = function(): void {
    ...
    if (totalItems > 0) {
        ...
        const totalPriceSpan = document.getElementById("total-price") as HTMLSpanElement | null;
        if (totalPriceSpan) {
        ...
        if (checkoutForm) {
            checkoutForm.addEventListener("submit", function(event: Event): void {
                event.preventDefault();
                const currentCart: BookItem[] = JSON.parse(localStorage.getItem("cart") || "[]");
