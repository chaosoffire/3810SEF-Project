...
export const signUp = async (
    ...
    try {
        ...
        const response: Response = await fetch(
            `${req.protocol}://${req.get("host")}/api/${apiVersion!}/user/register`,
            {
                method: "POST",
                headers: header,
        ...
        if (!req.body.admin) {
            if (response.status === 201) {
                ...
                const result =
                    (await response.json()) as interfaces.signUpError;
        ...
        if (!e.admin) {
            res.redirect(
                `/page/credential?signUpSuccess=false&message=${message}`,
            );
        } else {
