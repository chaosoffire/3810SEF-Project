...
export const addBook = async (req: express.Request, res: express.Response) => {
    const { title, author, description, price, categories, publishedYear } =
        req.body;
    ...
    if (req.file) {
        ...
        if (dimensions.width > 1000 || dimensions.height > 1600) {
            return res.status(400).json({
                success: false,
                message: "Image dimensions should not exceed 1000x1600 pixels.",
            });
        }
        ...
        if (aspectRatio < 0.6 || aspectRatio > 0.7) {
            ...
            return res.status(400).json({
                success: false,
                message: "Image aspect ratio should be approximately 2:3.",
            });
        }
        // Convert to base64
    ...
    try {
        ...
        return res.status(e.status).json({
            success: false,
            message: e.message || "Something went wrong, try a smaller image.",
        });
    }
};
