...
export const renderContent = async (
	...
	if (req.query.state === "home") {
		...
		const storedQuery: string = str.replace(patternForOffsets, "");
		...
		const response: Response = await fetch(
			`${req.protocol}://${req.get("host")}/api/${apiVersion}/book/${req.query.id}`,
			{
				method: "GET",
				headers: {
		...
		try {
			const ownBookListResponse: Response = await fetch(
				`${req.protocol}://${req.get("host")}/api/${apiVersion}/user/ownbooks`,
				{
					method: "GET",
					headers: {
						Cookie: req.headers.cookie || "",
			...
			if (ownBookListResponse.ok) {
				const ownbooks: interfaces.ownBookResult =
					await ownBookListResponse.json();
				if (ownbooks.books.length > 0) {
					const total: number = ownbooks.books.length;
					const totalPage: number = Math.ceil(total / 10);
					const currentPage: number = req.query.current
						? parseInt(req.query.current as string)
						: 1;
					...
					const booksDetailsResponse: Response = await fetch(
						`${req.protocol}://${req.get("host")}/api/${apiVersion}/book?${query}`,
						{
							method: "GET",
							headers: {
								Cookie: req.headers.cookie || "",
					...
					if (booksDetailsResponse.ok) {
						const booksDetails: interfaces.bookResult =
							await booksDetailsResponse.json();
